...
// 模板系列英文原文 http://www.modernescpp.com/index.php/template-get-insight
// 模板的实例化过程可视化 https://cppinsights.io/
...
public:
    // 用初始化列表构造MyVector
    explicit MyVector(const std::initializer_list<T> &init_list) : m_value(init_list)
    {
    }
...
{
public:
    Matrix() { std::cout << "partial template specialization\n"; }
};
...
{
public:
    Matrix() { std::cout << "full template specialization\n"; }
};
...
int main()
{
    ...
    auto ret7 = compare(std::string("abc"), std::string("xyz"), [](auto &&first, auto &&second)
                        { return first.back() > second.front(); });
}
